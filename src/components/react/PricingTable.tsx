  ...
  return (
    <div className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-neutral-800">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-neutral-100">Live Cloud GPU Pricing</h2>
            <span className="text-xs px-2.5 py-0.5 rounded-full bg-neutral-800 text-neutral-300 border border-neutral-700/60 font-mono">
        ...
        <div className="flex items-center gap-1.5 bg-neutral-950 p-1 rounded-xl border border-neutral-800 text-xs">
          <Clock className="w-3.5 h-3.5 text-neutral-500 ml-1 mr-0.5" />
          <span className="text-neutral-400 font-medium hidden sm:inline">Billing Span:</span>
          {[
            { id: '1h', label: '1 Hour' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {lowestSpotDeal && (
          <div className="md:col-span-2 rounded-xl border border-neutral-800 bg-neutral-950/60 p-3.5 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="text-neutral-400 font-semibold uppercase tracking-wider text-[10px] block">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                <span className="text-[10px] text-neutral-400 block">Spot Rate</span>
                <span className="text-sm font-mono font-bold text-neutral-100">
                  ${(lowestSpotDeal.spotPricePerHour * durationMultiplier).toFixed(2)}
                  <span className="text-[10px] text-neutral-400 font-normal"> / {durationMode}</span>
                </span>
              </div>
              <a
                ...
                className="px-3 py-1.5 rounded-lg bg-white hover:bg-neutral-200 text-neutral-950 font-semibold text-xs transition-colors flex items-center gap-1.5"
              >
                <span>Rent Node</span>
        ...
        <div className="rounded-xl border border-neutral-800 bg-neutral-950/60 p-3.5 flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-neutral-900 border border-neutral-800 flex items-center justify-center shrink-0">
              <Calculator className="w-4 h-4 text-neutral-300" />
            </div>
            <div>
              <span className="text-neutral-200 font-semibold text-[11px] block">
                Buy vs. Rent TCO Lab
              </span>
              <span className="text-[10px] text-neutral-500">Hardware break-even simulator</span>
            </div>
          </div>
          <button
            onClick={() => setShowTcoModal(true)}
            className="px-3 py-1.5 rounded-lg border border-neutral-700 bg-neutral-900 hover:bg-neutral-800 text-neutral-200 font-medium text-xs transition-colors shrink-0"
          >
            Calculate TCO
          </button>
        </div>
      ...
      {showTcoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-xl bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-2xl space-y-5 text-xs">
            <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
              <div className="flex items-center gap-2">
                <Calculator className="w-4 h-4 text-neutral-300" />
                <h3 className="text-sm font-semibold text-white">On-Premises Hardware vs. Cloud TCO Simulator</h3>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[11px] font-semibold text-neutral-300 block mb-1">Target GPU Model:</label>
                <select
                  value={tcoGpuModel}
              ...
              <div>
                <label className="text-[11px] font-semibold text-neutral-300 block mb-1">GPU Quantity:</label>
                <div className="flex gap-2">
                  {[1, 2, 4].map((count) => (
                    <button
                      key={count}
                      onClick={() => setTcoGpuCount(count)}
                      className={`flex-1 py-2 font-mono font-bold rounded-xl border transition-colors ${
                        tcoGpuCount === count
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-3 rounded-xl bg-neutral-950 border border-neutral-800">
                      <span className="text-[10px] text-neutral-400 block">Upfront Hardware</span>
                    ...
                    <div className="p-3 rounded-xl bg-neutral-950 border border-neutral-800">
                      <span className="text-[10px] text-neutral-400 block">Monthly Electricity</span>
                    ...
                    <div className="p-3 rounded-xl bg-neutral-950 border border-neutral-800">
                      <span className="text-[10px] text-neutral-400 block">Cloud Rent (Monthly)</span>
                      <span className="text-base font-bold font-mono text-neutral-300">
                  ...
                  <div className="p-4 rounded-xl bg-neutral-950 border border-neutral-800 space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="font-semibold text-neutral-200">Break-Even Horizon:</span>
                      <span className="text-base font-mono font-bold text-white">
                        {res.breakEvenMonths} Months
                      </span>
                    </div>
                    <p className="text-[11px] text-neutral-400 leading-relaxed">
                      If running 24/7 workloads, purchasing hardware pays for itself after <b className="text-neutral-200">{res.breakEvenMonths} months</b>.
                      Estimated 1-year net savings: <b className="text-neutral-200">${res.savings1Year.toLocaleString()}</b> (2-year: <b className="text-neutral-200">${res.savings2Year.toLocaleString()}</b>).
                    </p>
                  </div>
                </div>
              );
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setShowTcoModal(false)}
                className="px-4 py-2 bg-white hover:bg-neutral-200 text-neutral-950 rounded-xl font-semibold transition-colors"
              >
                Done
              </button>
            </div>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-neutral-950 border border-neutral-800 rounded-xl text-xs text-neutral-200 placeholder-neutral-500 focus:outline-none focus:border-neutral-600"
            />
          </div>
          ...
          >
            <CheckCircle className={`w-3.5 h-3.5 ${onlyCompatible ? 'text-neutral-200' : 'text-neutral-500'}`} />
            <span>≥ {requiredVramGb} GB VRAM Only</span>
          </button>

          {/* GPU Count Filter */}
          <div className="flex items-center rounded-xl bg-neutral-950 border border-neutral-800 p-1 text-xs">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
          ...
          <span className="text-[11px] text-neutral-400 mr-1 hidden sm:inline">Arch:</span>
          {architectures.map((a) => {
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[540px] rounded-xl border border-neutral-800 bg-neutral-950/70 relative">
        <table className="w-full text-left border-collapse text-xs">
          <thead className="sticky top-0 z-20 bg-neutral-900 border-b border-neutral-800">
            <tr className="text-neutral-300 font-semibold select-none">
              ...
              <th
                className="py-3 px-3 hover:text-white transition-colors text-right hidden lg:table-cell"
              >
                <div className="flex items-center justify-end gap-1">
              ...
              <tr>
                <td colSpan={8} className="py-12 text-center text-neutral-500">
                  <div className="max-w-xs mx-auto space-y-2">
                    ...
                    <p className="font-medium text-neutral-400">No matching GPU nodes found</p>
                    ...
                    <button
                      ...
                      className="mt-2 px-3 py-1 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 rounded-lg text-xs transition-colors"
                    >
                      Reset All Filters
                    </button>
                  </div>
                ...
                return (
                  ...
                  >
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-neutral-100">{gpu.gpuModel}</span>
                            {gpu.popular && (
                          ...
                          <div className="flex items-center gap-2 text-[10px] text-neutral-500 mt-0.5">
                            <span className="text-neutral-400">{gpu.architecture}</span>
                            <span>•</span>
                    ...
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-neutral-900 text-neutral-300 border border-neutral-800">
                        {gpu.provider}
                      </span>
                    </td>
                      ...
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`font-mono font-semibold text-xs ${
                            isCompatible ? 'text-neutral-100' : 'text-neutral-500'
                          }`}
                        >
                          {gpu.totalVramGb} GB
                        </span>
                    ...
                    {/* On-Demand Price */}
                    <td className="py-3 px-3 text-right">
                      <span className="font-mono font-medium text-neutral-300 text-xs">
                    ...
                    <td className="py-3 px-4 text-center">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-lg border border-neutral-700 bg-neutral-900 hover:bg-neutral-800 text-neutral-100 font-medium text-xs transition-colors"
                      >
                        <span>Rent</span>
                        <ExternalLink className="w-3 h-3" />
                      </a>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-neutral-500 pt-2">
        <div className="flex items-center gap-1.5">
